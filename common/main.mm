...
extern uint32_t kWindowIconWidth;
extern uint32_t kWindowIconHeight;
extern uint8_t kWindowIconBytes[];
...
void keyCallback(GLFWwindow *window, int key, int scancode, int action,
  ...
  case GLFW_KEY_V: {
    if (action == GLFW_RELEASE) {
      ...
      layer.displaySyncEnabled = !layer.displaySyncEnabled;
      std::cout << "Enable VSync = " << (int)layer.displaySyncEnabled
                << std::endl;
    }
    break;
  }
  ...
  default: {
    getApp()->onKeyButton(key, scancode, action != GLFW_RELEASE);
  }
  }
}
...
void mouseCallback(GLFWwindow *window, int button, int action, int mods) {
  ...
  getApp()->onMouseButton(xpos, ypos, button, action != GLFW_RELEASE);
}

void mouseMoveCallback(GLFWwindow *window, double xpos, double ypos) {
  getApp()->onMouseMove(xpos, ypos);
}

void resizeCallback(GLFWwindow *window, int, int) {
  int fbWidth = 0, fbHeight = 0;
  glfwGetFramebufferSize(window, &fbWidth, &fbHeight);
  getApp()->onResize(static_cast<uint32_t>(fbWidth),
...
int main(int argc, char **argv) {
  ...
  if (result["w"].count() != 0 && result["h"].count() != 0) {
    cmdWindowSize =
        std::make_pair(result["w"].as<uint32_t>(), result["h"].as<uint32_t>());
  }
  ...
  auto [width, height] =
      cmdWindowSize.value_or(std::make_pair(mode->width, mode->height));

  // Create a window.
  GLFWwindow *window = glfwCreateWindow(
      width, height, getApp()->getName(), nullptr, nullptr);

  // Make the window to cover full screen on the current monitor and get content
  // scale.
  float xscale = 1.0f, yscale = 1.0f;
  if (auto currentMonitor = getCurrentMonitor(window);
  ...
  if (!getApp()->onInitialize((__bridge MTL::Device *)device,
  ...
  while (!glfwWindowShouldClose(window) && !needClose) {
    ...
    @autoreleasepool {
      ...
      getApp()->onMainLoopTick((__bridge CA::MetalDrawable *)drawable,
                               elapsed / 1000.0);
    }
